...
**The Fix:** Using `localStorage.setItem('doodle_user', JSON.stringify({username, color}))`. This allows the newly loaded `<GameRoom/>` to fetch who you are immediately on mount to establish your Socket identity. Note that we store both the identity _and_ the Auth Token!
